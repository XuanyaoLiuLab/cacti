...
The `cacti` package implements the **CACTI peak-based pipeline** for chromatin QTL mapping.  

It is designed to:

- group nearby peaks into **non-overlapping windows**,  
- residualize peak intensities with respect to covariates,  
- test association between genetic variants and **multi-peak windows** using a principal-component omnibus (PCO) test, and  
...
1. **Option 1 (existing workflow): QTL summary stats already available**  
   - Use your existing per-chromosome QTL summary stats file(s) with required columns:
     `phe_id`, `var_id`, `z`.
   - Pass `qtl_file` / `qtl_files` to `cacti_run_chr()` / `cacti_run_genome()`.
...
2. **Option 2 (new workflow): QTL summary stats not available**  
   ...
   - In `cacti_run_chr()` / `cacti_run_genome()`, set `qtl_file = NULL` / `qtl_files = NULL`
     and provide:
...
In this case, call `cacti_run_chr()` / `cacti_run_genome()` with
`qtl_file = NULL` / `qtl_files = NULL`.

```{r show-vcf-header, eval=FALSE}
...
Set `qtl_file = NULL`, then provide genotype input (`file_vcf` or `file_geno` + `file_snp_pos`).
CACTI will run MatrixEQTL first and then continue to the peak-window test.

```{r example-run-chr5-with-matrixqtl, eval=FALSE}
library(cacti)
...
When QTL summary stats are not precomputed, you can use the same pattern with
`qtl_files = NULL` and genotype input (`file_vcf` or `file_geno` + `file_snp_pos`);
`cacti_run_genome()` will run MatrixEQTL first, then run CACTI.
